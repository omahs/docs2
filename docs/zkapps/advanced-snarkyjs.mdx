...
To allow for proof authorization by the token owner, a token zkApp should provide a `authorize` method that inspects any changes that another account wishes to make and then authorizes the transaction if the changes are valid. To authorize a transaction, you can use the `this.experimental.authorize` method within a zkApp.

By using `this.experimental.authorize` inside your zkApp, a token owner can authorize a transaction either by inspecting a specified `AccountUpdate` or by calling a specified `SmartContract` method and inspecting the results.
...
class TokenContract extends SmartContract {
  ...
  /**
   * This method is used to authorize a transaction between two accounts
   * by calling a specified SmartContract method as `callback`
   */
  @method sendTokens(
    senderAddress: PublicKey,
    receiverAddress: PublicKey,
    ...
    this.experimental.authorize(callback); // Authorize the specified @method from another zkApp
...
tx = await Local.transaction(feePayer, () => {
  // Create a 'callback' that authorizes a transaction by calling the `authorizeSend` method on zkAppB
  ...
  tokenZkApp.sendTokens(zkAppBAddress, account1Address, authorizeSendingCallback);
});
await tx.prove();
await tx.send();
```
